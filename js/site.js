  ...
  async function store() {
    ...
    grid.innerHTML = C.ranks.map(r => `
      <article class="rank-card ${r.featured?"featured":""} reveal">
        ...
        <ul>${r.perks.map(p=>`<li>${p}</li>`).join("")}</ul>
        <div class="actions">
          <button class="btn ${r.featured?"btn-primary":"btn-secondary"} checkout-btn" data-rank="${r.name}">
            BUY ${r.name}
          </button>
        </div>
      </article>`).join("");
    ...
    if (notice && purchaseState === "success") {
      notice.hidden = false;
      notice.textContent = "Purchase complete — thank you for supporting Vision SMP!";
    } else if (notice && purchaseState === "cancelled") {
      notice.hidden = false;
      notice.textContent = "Checkout cancelled. Nothing was charged.";
    }
    ...
    $$(".checkout-btn").forEach(button => {
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          const redirectUrl = data.redirect_url || data.auth_url || data.checkout_url;
